...
RAZO_B = read.csv("RAZO_B.csv")

# Number of fledglings 
RAZO_J = read.csv("RAZO_J.csv")

# RAZO count data
RAZO_count = read.csv("RAZO_count.csv")

# Load CMR data (encounter histories)
RAZO_EH = read.csv("RAZO_EH.csv")

# Load # times resighted as covariate
cov = read.csv("RAZO_cov.csv")

# Create encounter history matrix to be used in model
RAZO_df = RAZO_EH %>% select("BirdID","Contact_Year","Age") %>% mutate(seen=1)
ch=RAZO_df %>% 
  group_by(BirdID) %>%
...
f.j <- apply(j_ch, 1, get.first)
f.a <- apply(a_ch, 1, get.first)

# Compute vector with occasion of last capture
get.last <- function(x) max(which(x!=0))
l.j <- apply(j_ch, 1, get.last)
l.a <- apply(a_ch, 1, get.last)
...
z.dat.j = known.state.cjs(j_ch,f.j,l.j)
z.dat.a = known.state.cjs(a_ch,f.a,l.a)
...
########################################################################################
# Integrated population for Razorbills breeding on Machias Seal Island (1998-2019)
# Model description
...
for (t in 1:(n.occasions-1)){
   alpha.star[u,t] ~ dnorm(0, 1)T(-5,5)      # Priors for age and time specific survival (bounded to help with convergence)
   logit(alpha[u,t]) = mu.phi[u] + alpha.star[u,t] * sigma.alpha[u]
   }
}

# Constrain survival after first year to be same as adults
...
# Priors for gamma (availability for detection given absense/presence at previous time step)  
  for (j in 1:4){
...
# Productivity data (Binomial regression model) 
for (t in 1:n.occasions){   
...
RAZO_IPM_data <- list(z = z.dat, 
                      ...
                      age=age) 
# MCMC settings
ni <- 500000; nb <- 150000; nc <- 3; nt <- 5; na <- 25000

# Call JAGS, check convergence and summarize posteriors
RAZO_IPM_out <- jags(RAZO_IPM_data, inits=inits, parameters, "RAZO_IPM.txt", n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel = TRUE)

# Print Results
print(RAZO_IPM_out, digits = 3)

# Examine traceplots
MCMCtrace(RAZO_IPM_out,
          pdf = FALSE,
          ind = TRUE,
          Rhat = TRUE,
          n.eff = TRUE)
...
RAZO_CMR_out <- jags(RAZO_CMR_data, inits=inits, parameters, "RAZO_CMR.txt", n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel = TRUE)

# Print Results
print(RAZO_CMR_out, digits = 3)

# Examine traceplots
MCMCtrace(RAZO_CMR_out,
          pdf = FALSE,
          ind = TRUE,
          Rhat = TRUE,
          n.eff = TRUE)
...
# Parameters monitored
parameters <- c("fecund","mfec","sigma.fec2")

RAZO_prod_data <- list(n.occasions = length(RAZO_J),
                           J=RAZO_J, 
                           nbrood=RAZO_B)
# MCMC settings
ni <- 500000; nb <- 150000; nc <- 3; nt <- 5; na <- 25000
...
# Call JAGS (ART 2 min), check convergence and summarize posteriors
RAZO_prod_out <- jags(RAZO_prod_data, inits=NULL, parameters, "RAZO_prod.txt", n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel = TRUE)

# Print Results
print(RAZO_prod_out, digits = 3)

# Examine traceplots
MCMCtrace(RAZO_prod_out,
          pdf = FALSE,
          ind = TRUE,
          Rhat = TRUE,
          n.eff = TRUE)
...
n.draws <- RAZO_IPM_out$mcmc.info$n.samples
draws <- RAZO_IPM_out$sims.list
...
n.years <- length(RAZO_IPM_out$mean$alpha[1,])
...
RAZO_tLTRE_df1 = rbind(CRI,mean) 

# Change row names
rownames(RAZO_tLTRE_df1) <- c("ll_value","ul_value","mean_value")

# Transpose dataframe
RAZO_tLTRE_df1 = as.data.frame(t(RAZO_tLTRE_df1))

# Make demographic rate a type column
RAZO_tLTRE_df1 <- RAZO_tLTRE_df1  %>%
...
RAZO_tLTRE_df1$type <- factor(RAZO_tLTRE_df1$type, levels = desired_order)
...
RAZO_tLTRE_df1_2=data.frame(type=c("phij","phia","f","n1","n2","n3","n4","n5"),
...
# Create dataframe
RAZO_tLTRE_df2 = as.data.frame(vcov_contribution)

# Change row names
rownames(RAZO_tLTRE_df2) <- c("Variance","Covariance")

# Add a column for variance or covariance
RAZO_tLTRE_df2$vcov <- row.names(RAZO_tLTRE_df2)

# Reshape the data to long format
RAZO_tLTRE_df2 <-RAZO_tLTRE_df2 %>%
...
RAZO_tLTRE_df2$Demo_rate <- factor(RAZO_tLTRE_df2$Demo_rate, levels = desired_order)

#Pull out CRI data from first df and add to new df
RAZO_tLTRE_df2$ll_data = c(RAZO_tLTRE_df1$ll_value, rep(NA, 8))
RAZO_tLTRE_df2$ul_data = c(RAZO_tLTRE_df1$ul_value, rep(NA, 8))
...
# Get mean and 95% CRI for proportional contribution of each demographic parameter
# and create dataframe
RAZO_tLTRE_df2_2=data.frame(type=c("phij","phia","f","n1","n2","n3","n4","n5"),
...
RAZO_tLTRE_df2_2<- pivot_longer(
  data = RAZO_tLTRE_df2_2,
...
RAZO_tLTRE_df3 = as.data.frame(cbind(juv_ad_tc,juv_fec_tc,ad_fec_tc))

# Reformat dataframe 
RAZO_tLTRE_df3 <- RAZO_tLTRE_df3   %>%
...
# Bind together in dataframe
RAZO_tLTRE_df3_2 = as.data.frame(cbind(juv_var,ad_var,fec_var))

# Reformat dataframe 
RAZO_tLTRE_df3_2  <- RAZO_tLTRE_df3_2   %>%
  gather(key = "type", value = "value")
...
# Bind together in dataframe
RAZO_tLTRE_df4 = as.data.frame(cbind(ad_juv_pv,juv_fec_pv,ad_fec_pv))

# Reformat dataframe 
RAZO_tLTRE_df4  <- RAZO_tLTRE_df4   %>%
  gather(key = "type", value = "value")


# Calculate mean, 2.5% quantile, and 97.5% quantile for each type
RAZO_ss_pc <- tapply(RAZO_tLTRE_df4$value, RAZO_tLTRE_df4$type, function(x) {
  mean_value <- mean(x)
  ll <- quantile(x, 0.025)
  ul <- quantile(x, 0.975)
...
RAZO_tLTRE_df5 = as.data.frame(cbind(b_year,diffs))

# Covert year to factor
RAZO_tLTRE_df5$b_year <- as.factor(RAZO_tLTRE_df5$b_year)


# Get contribution of the changes in demographic rates and of population structure
# Make V a dataframe
RAZO_tLTRE_df6 = as.data.frame(V) 

# Rename columns to reflect beginning year
RAZO_tLTRE_df6 = RAZO_tLTRE_df6 %>% dplyr::rename(
